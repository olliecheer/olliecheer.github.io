...
void bubble_sort(vec<int> &nums) {  
  ...
  // We loop (nr-1) times.  
  // Each time, the largest element in this loop  // will be placed at its final index.  for (int i = 0; i < nr - 1; i++)  
...
void selection_sort(vec<int> &nums) {  
  ...
  // and place it at the very first index in this loop.  for (int i = 0; i < nr - 1; i++) {  
    int minimal_idx = i;  
