...
public:
  ...
  int check(const vec<int> &arr, int m, int a1, int a2, int mod1, int mod2) {
    int n = arr.size();
    ...
    for (int start = 1; start <= n - m; ++start) {
      h1 = (h1 * a1 % mod1 - arr[start - 1] * aL1 % mod1 + arr[start + m - 1]) %
           mod1;
      h2 = (h2 * a2 % mod2 - arr[start - 1] * aL2 % mod2 + arr[start + m - 1]) %
           mod2;
      if (h1 < 0)
        h1 += mod1;
